...
def main():
    ...
    if all_channels:
        ...
        if all_token:
            ...
            if not args.no_epg:
                # plex_all.xml is skipped — combining all regions exceeds GitHub's
                # 100 MB file size limit. Use per-region .xml.gz files instead.
                logger.info("Skipping plex_all.xml (too large for GitHub). Use per-region .xml.gz files.")
